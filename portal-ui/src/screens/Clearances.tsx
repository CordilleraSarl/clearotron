...
export function Clearances({ ctx }: { readonly ctx: ShellContext }) {
  // The list's own width, which decides whose column shares it is drawn with: the board's where they hold,
  // the narrow ones below that (contract/listView.ts).
  // A callback ref, because the wrapper is not drawn until the list has loaded.
