...
//
//   Stopped at common-law-half:b. invalid_file:clearance-search/tmpe2er1-vibrante-frostplum/
...
const REAL_REASON =
  'invalid_file:clearance-search/tmpe2er1-vibrante-frostplum/2026-08-02-fixture/common-law-findings.half-b.md:connotation_undisposed:VIBRANTE FROSTPLUM urban dictionary,FROSTPLUM meaning slang,FR'
...
test('NO PATH, TEMP DIR, RUN SLUG, STAGE ID, FILE NAME OR ERROR ENUM reaches a rendered line', () => {
  ...
  const cases: [string | null, string | null][] = [
    ...
    ['synthesis', 'coverage_ledger_unparseable:clearance-search/tmpxx-mark/2026-08-02-fixture/grid.json'],
...
test('an unrecognised reason falls back to the engine\'s words ONLY when they are safe to render', () => {
  ...
  const unsafe = readableFailure('synthesis', 'weird_new_token:clearance-search/tmpzz/2026-08-02-x/grid.json')
  assert.equal(unsafe.detail, null, 'a path-carrying unknown goes behind the disclosure, not onto the row')
  assert.equal(unsafe.raw, 'weird_new_token:clearance-search/tmpzz/2026-08-02-x/grid.json', 'but is not lost')
