...
test('the control is ON the read row, and the read row is what renders it', () => {
...
test('both retires are gated by the SAME capability expression', () => {
...
//
// Ruling, relayed on the issue: "Failed runs on clearance screen - no." The build shape is
// removal from the default view; the placement of their remaining home was left to the builder.

test('c5 a failed name is filtered out by the MARK, not by the read', () => {
...
test('c5 the failed names keep exactly one route in', () => {
...
test('c5 `failed` stays in TERMINAL, or the older bug comes back', () => {
...
test('c5 cancelled is NOT swept up with failed', () => {
