...
// WHAT A TERRITORY ALREADY COVERS, on the surface where the search is composed. A requester who asked
// to clear a mark in one EU member state was sold that state AND the EU as two territories — which
// resolves a different product, one carrying no case-law reading and no automatic native-language
// investigation — because nothing here said that the member state already carries the EU-wide right.
// The engine has searched on that basis since the ruling driver/binding-layers.mjs quotes; this is the
// same fact, said where the composition happens. It states what BINDS a territory, never what a
// particular register will return: plan_run's `scope.bindingLayers` reports the first per territory.
const TERRITORY_DESC = "Name only the territories the requester actually wants cleared. A country already "
  + "carries the regional register that binds it and international registrations designating either — an EU "
  + "member state is bound by its national register, the EU-wide register and international registrations, "
  + "and a clearance for that country covers all three. So DO NOT add the region as a second territory to "
  + "\"include\" it: two territories resolve to a multi-country focus search, which carries no case-law or "
  + "opposition reading and no automatic native-language investigation, so the wider-looking list buys less. "
  + "If the requester asks for the country AND its region anyway, say what it costs before composing it, and "
  + "never quietly rewrite what they asked for. plan_run's scope.bindingLayers lists what each territory binds.";

// ONE definition for both doors. plan_run takes the SAME arguments as start_run, so a field described
// on one and not the other teaches an assistant to compose the preview differently from the run it is
// previewing — which is how the preview stopped mentioning the authoritative list, the account default
// and the worldwide switch.
const JURISDICTIONS_DESC = "WHERE the search points — and, for a clearance, WHICH search it is: one country "
  + "is a full-country-search, a region or two-or-more countries a multi-country-focus-search. AUTHORITATIVE "
  + "(the matter frame is told not to widen past them). Omit \u21d2 the project/customer default territories; "
  + "send worldwide:true to search everywhere instead. Names or codes both read; max 20. " + TERRITORY_DESC;
