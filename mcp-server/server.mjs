...
const tools = {
  ...
  async what_if_plan({ runId, stage, axis, instructions, model }) {
    const run = mustRun(runId);
    const { whatIfPlan } = await import("./lib/whatif.mjs");
    return whatIfPlan({ run, stage, axis, instructions, model });
  },
...
const TOOL_DEFS = [
  ...
  { name: "what_if_plan", description: "DRY-RUN a single-step what-if (no spend): what re-runs, the cost prior, what downstream is NOT recomputed, a completeness verdict, and a confirmationToken. Express the change via instructions (e.g. 'treat ACME as expired') and/or model. Live runs only.", inputSchema: { type: "object", properties: { ...runIdProp, stage: { type: "string" }, axis: { type: "string" }, instructions: { type: "string" }, model: { type: "string" } }, required: ["runId", "stage"] } },
