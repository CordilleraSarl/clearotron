...
export const REGISTER_SERVERS = {
  ...
  //     ordered the mint UNCONDITIONALLY — `stages.mjs` (the unconditional mint the register-units digest orders) — so a signa deploy was told to
  ...
  if (localKeys.has("register")) {
    ...
    if (!runDir) {
      throw new Error("gather-config: a register server needs the run it is fetching for — record bodies "
        + "belong to their run since #743, and the box-global ledger is retired. Pass runDir.");
    }
  }
