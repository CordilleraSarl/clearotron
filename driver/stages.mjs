...
import { frameworkFor, workedExamplesFor } from "./framework.mjs"; import { methodDictation, inputsShape } from "./framework-method.mjs";
...
export function paths(runDir) {
  ...
  return {
    ...
    // `<runDir>/_driver/**` a tree a seat may NEVER write into (`authorityTrees()` in authority-trees.mjs, `live: true`) —
...
export const OWNER_SWEEP_STEERING = `OWNER COVERAGE on this lane: an owner's marks that share the client's word or family, in the client's classes and markets, are asked with the owner×term slice — a mark-text proposal carrying the owner as a scope field, e.g. {predicate:"exact"|"default", term:"<the mark/element>", owner:"<the owner>", nice_classes:[…]}. That intersects the owner's portfolio with the dangerous band in ONE query and ENUMERATES record-by-record even where the bare portfolio is a many-thousand crowd. Asking more of an owner is a supplemental that states its question about that owner: can it block, would it, what kind of filer it is. Never because it is an incumbent or on the watchlist. The plan does not search a guessed owner list. Owners come from the records the searches returned. Propose an owner slice only where an owner's other marks could change the assessment, for example when the records show a family of marks sharing the element. A bare predicate:"owner" proposal (the owner name as the term; nice_classes REQUIRED — an all-class owner sweep is refused) is CROWD CONTEXT, not coverage: an honest "incomplete" COUNT descriptor that sizes the portfolio and whose write-up POINTS AT the owner×term slice qids that actually cover it — never "portfolio too large, noted" as an ending in itself. register_search {owner, name} stays what it is: a COUNT-ONLY context probe (limit:1), a count, never a review pass. And you NEVER stand sampled register_search pages in for either — not for a slice, not for a sweep — nor write such a sample up as though the owner had been screened: ten of an owner's 432 hits presented as review is a recall hole with a clean face, not a search. "Portfolio too large, noted" is never a finding: size the crowd, then cover it with owner×term slices.`;
...
export const STAGES = {
  ...
  synthesis: {
    ...
    message: ({ paths: P, job, customerUnknown, profile, intakeAsks, enforcerSignals, framework, frameworkMethod, jxAim, registerOnly, crowdContext, dispatchBlocks, findingsSurface, depth }) => lines(
      ...
      // (stripFrontMatterBlock in predelivery-lint.mjs), so a coverage number re-typed into prose
      ...
      ...(Array.isArray(findingsSurface) && findingsSurface.length ? [
        ...
        `Decline by calling the \`record_declination\` tool. Its schema names the fields it takes and what each is for — read them there, not here. The one thing to know before you call: you cite a record or page by its POSITION in the list below — there is no field for a mark name, a uri or a page address, so anything you were not handed cannot be expressed at all. The reason vocabulary is closed: ${DECLINATION_REASON_TOKENS.join(" / ")}, each an omission synthesis-rules.md already authorises, and if your ground is none of them the rules do not let you omit the record. ONE OF THEM IS NOT AVAILABLE ON A LIVE RECORD IN AN INSTRUCTED CLASS: \`not-worth-the-line\` is a statement about the report's budget rather than about the mark, and a live registration inside one of the matter's own classes is never omitted for want of room — carry it, or decline it on something true of THAT MARK. The call is refused if you try, so it costs you a turn. The grounds you write are one or two lines in your OWN words on why THIS record does not earn a line — never machine-parsed, and what the reviewing lawyer reads. Send them in one batch where you can; a refused row never voids its neighbours, and the answer names what is still undecided so you can finish in this turn.`,
        ...
        ...findingsSurface.flatMap((r, i) => r?.kind === "page" ? [] : [`  ${i}. ${r.mark ?? "(unnamed)"}${r.owner ? ` — ${r.owner}` : ""}${r.tier ? ` [${r.tier}]` : ""}${r.uri ? ` ${r.uri}` : ""}`]), ...(findingsSurface.some((r) => r?.kind === "page") ? [`The pages the web notes marked as candidates or conflicts, continuing the same positions:`, ...findingsSurface.flatMap((r, i) => r?.kind === "page" ? [`  ${i}. ${r.url ?? r.page}`] : [])] : []),
      ] : []),
      ...
      // VOICE / PRIORITISATION (the Korvane-headline fix). Lead with the genuine top risk, not a partner by default.
  ...
  "report-overview": {
    ...
    message: ({ paths: P, job, customerUnknown, profile, intakeAsks, displayVerdict, registerOnly, searchPolicy }) => lines(
      ...
      `TRANSFORM, NOT FREE-GENERATION: overall_caption and # Actions draw ONLY on the settled synthesis narrative ${P.narrative} (and findings.json ${P.findings}) — introduce NO finding, owner, or claim that is not already there, NEVER re-compute a band, and never let a claim outrun its evidence (a verified-from-record fact stays fact; an inferred-from-signal claim reads AS an inference — "on the available signals…" — never flattened to bald fact). WHERE THE SETTLED NARRATIVE REASONED MORE THAN ONE RATIFIED FORM OF THE NAME, CARRY ITS PER-FORM READ THROUGH — the overall_caption says which form the decisive conflicts turn on, or that the reads are the same for both, in the narrative's own words. Do not flatten two reads into one undifferentiated statement: that loses the distinction the client ratified both forms to get. This is a transform of what the narrative already settled, not a second rating, and the band is unchanged — one overall band as today.`,
...
export const supplementalLaneResumeLine = (P, axis) =>
  `Any ADDITIONAL register sub-query this needs: PROPOSE it via register_propose_supplemental ({"axis": "${axis}", "output_path": ${JSON.stringify(P.registerBand(axis))}, "proposals": […]}) — the tool executes it and merges the band itself. You never run register coverage via register_enumerate and never author band blocks. ${SUPPLEMENTAL_LANE_STEERING}`;
