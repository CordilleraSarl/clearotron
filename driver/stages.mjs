...
import { resolveTerritories, defaultJurisdictionsLine, narrowingOrderLine } from "./effective-scope.mjs";   // the ONE territory ladder (the geography stamp included) + the defaults-never-widen rule (2160)
...
export const AGENT_WHATSAPP = (() => {
  ...
  return { localagent: "+10000000001", clawdi: "+10000000001", "clawdi-alex": "+10000000002", "relay-agent": "+10000000003" };
})();
...
export const STAGES = {
  ...
  "register-unit": {
    ...
    message: ({ paths: P, axis, job, profile, registerPlan, capabilities = registerCapabilities() }) => {
  ...
  "case-law": {
    ...
    message: ({ paths: P, findingsIndex, caseLawSources }) => lines(
      ...
      `ALSO write the RETRIEVAL RECORD to ${P.caseLawCitations} — a JSON OBJECT with EXACTLY these keys: {"schema_version":2,"queries":[{"query":"<the search you dispatched, verbatim>","jurisdiction":"<the territory it was scoped to>","results":<how many hits it returned, or null when the query never reached a source — never 0 for a query you could not send>}, …],"citations":[{"proceeding":"<case name or number as published>","forum":"<court or office>","jurisdiction":"<territory>","decided":"<YYYY-MM-DD or YYYY>","url":"<the link you ACTUALLY fetched>","read":"read"|"listed-not-read"|"unreachable","ord":<the finding ordinal this grounds, or null>,"bearing":"<one line: what it grounds>"}, …]}.`,
