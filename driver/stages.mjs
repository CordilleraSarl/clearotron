    ...
    return lines(
      ...
      planEntries.length
        ? lines(
            ...
            `EXECUTE THE FROZEN PLAN VIA THE TOOL: call register_execute_plan ONCE with {"plan_path": ${JSON.stringify(P.registerPlan)}, "axis": "${axis}", "output_path": ${JSON.stringify(P.registerBand(axis))}}. The tool runs every dictated entry below ITSELF (paged enumerates; count-only crowd descriptors; a "when"-guarded fringe only if its parent enumerated) and WRITES the band file itself with each block's qid stamped. Do NOT run these dictated entries manually and do NOT write their blocks yourself.`,
  ...
  synthesis: {
    ...
    message: ({ paths: P, job, customerUnknown, profile, intakeAsks, enforcerSignals, framework, jxAim, registerOnly, crowdContext, dispatchBlocks, findingsSurface, depth }) => lines(
      ...
      ...(Array.isArray(findingsSurface) && findingsSurface.length ? [
        `DECLINATIONS (MANDATORY): the register digest carried ${findingsSurface.filter((r) => r?.kind !== "page").length} record(s) onto your findings surface${findingsSurface.some((r) => r?.kind === "page") ? `, and the web notes surfaced ${findingsSurface.filter((r) => r?.kind === "page").length} page(s)` : ""}. Each one leaves this stage by one of two routes and there is no third: it becomes a finding in ${P.findings}, or you decline it BY NAME with a reason and a ground. A record you simply do not mention is reported as a defect of this run, named individually in its trace and shipped as an open doubt, so silence costs you more than a declination ever will.${findingsSurface.some((r) => r?.kind === "page") ? " A page you do not mention is counted as a defect of this run." : ""}`,
        ...
        ...findingsSurface.flatMap((r, i) => r?.kind === "page" ? [] : [`  ${i}. ${r.mark ?? "(unnamed)"}${r.owner ? ` — ${r.owner}` : ""}${r.tier ? ` [${r.tier}]` : ""}${r.uri ? ` ${r.uri}` : ""}`]), ...(findingsSurface.some((r) => r?.kind === "page") ? [`The pages the web notes surfaced, continuing the same positions:`, ...findingsSurface.flatMap((r, i) => r?.kind === "page" ? [`  ${i}. ${r.url ?? r.page}`] : [])] : []),
      ] : []),
