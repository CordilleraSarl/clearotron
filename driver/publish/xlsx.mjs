...
export async function buildAudit(contract, auditParsed, outPath, mark = '', fm = {}) {
  ...
  addSheet(wb, 'Coverage & gaps', COVERAGE_COLS,
    [...coverageRows(coverage), ...coverageRows(contract?.droppedConditions || []),
     ...coverageRows(contract?.undispatchedProbes || []), ...coverageRows(contract?.withheldFamilies || []), ...(contract?.setAside || []).map((s) => ({ Area: plainNote(s.area), State: 'Note', 'What was done': plainNote(s.note) || '—', "What's left": '—' }))], (row, _d, kept) => {
