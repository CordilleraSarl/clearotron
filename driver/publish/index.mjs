...
import { buildAudit } from './xlsx.mjs'; import { frameSetAsideRows } from '../web-grid.mjs';   // — and the stores the matter frame set aside, with its reason
...
export async function publishReport({ runId, codename, reportMd, auditMd, findingsJson, poolRoot, poolUrl, customerKey, delivery, runDir, skipRegen = false }) {
  ...
  const withheldFamilies = [...withheldFamilyRows(runDir ?? dirname(reportMd)), ...frameSetAsideRows(runDir ?? dirname(reportMd))];
