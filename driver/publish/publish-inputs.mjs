...
export const PUBLISH_INPUTS = {
  ...
  // The recall net's receipt, carried only by runs from before the recall store's removal. Its
  // overflow[] carries the probes such a run minted and did not dispatch, one row each on the
  // workbook's gaps sheet. `optional`: no run after the removal writes one, and before it the lane was
  // env-gated and minted nothing for a matter with no remembered conflict, so an absent receipt is a
  // run that had nothing to record rather than one that lost it.
