...
export const E3_BACKLOG = [
...

  {
    stage: "register-unit",
    kind: "literal-json-skeleton",
    where: "driver/stages.mjs:2251 (the non-supplemental-lane branch)",
    surface: "stage-message",
  ...
  },
  {
    ...
    reparsedBy: "driver/placement-model.mjs. AND IT IS STALE: #562 made placements.json driver-rendered, and stages.mjs:2408 says \"DO NOT WRITE placements.json (the driver renders it from this form)\" — the skill file the stage is ordered to \"read and follow exactly\" dictates the key set of a file the message forbids it to write. Two contracts in one dispatch",
    removedByMove: "NOTHING ON THE #850 PLAN REMOVES THIS",
  },
  {
    stage: "placement-inquiry",
  ...
  // records nobody ordered.
  {
    ...
    where: "driver/stages.mjs:2616 (the digest message) + driver/coverage-form.mjs (coverageFormBrief — the dispatch block carrying the enum and the row shape)",
    surface: "stage-message",
  ...
  // this commit for the same reason.
  {
    stage: "synthesis",
    kind: "dictated-line-shape",
    where: "driver/stages.mjs:3071 (restated at driver/skills/prelim-search/synthesis-rules.md:428)",
    surface: "stage-message",
  ...
  },
  {
    stage: "synthesis",
    kind: "dictated-line-shape",
    where: "driver/stages.mjs:3084 (restated at driver/skills/prelim-search/synthesis-rules.md:475)",
    surface: "stage-message",
  ...
  },
  {
    ...
    reparsedBy: "driver/verify.mjs:759 CORRECTION_ON_RE = /\\[on:\\s*([0-9,\\s-]*?)\\s*\\]/i. SKILL-FILE ONLY — the stage message at stages.mjs:1842-1877 never mentions `[on:]`. This is #850's \"the element shape is in the skill file, not the stage message\" in its purest form: an E3 lint reading stages.mjs alone sees the [kind:] token and misses its twin",
    removedByMove: "NOTHING ON THE #850 PLAN REMOVES THIS",
  },
  {
    stage: "narrative-refutation",
