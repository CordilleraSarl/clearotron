...
// (register-plan.mjs:858 compileRegisterPlan); the unreachable office produces no entry, so no qid, so no band block, so
// nothing ever reaches joinPlanToBands' deferred bucket — whose only source is a block stamped
// `error:true && deferred:true` (register-plan.mjs:1626 extendRegisterPlan).
//
