...
function trees() {
  ...
  for (const d of [join(baseDir, "clearance-register"), join(overlayDir, "clearance-register"), join(overlayDir, "clearance-search")])
...
test("SAFETY: while the overlay holds the file, resolution is unchanged — the migration is opt-in per file", () => {
  ...
  put(baseDir, "clearance-register/digest.md", "REPO COPY");
  const live = put(overlayDir, "clearance-register/digest.md", "LIVE COPY");
  const resolve = makeResolver({ overlayDir, baseDir });
  assert.equal(resolve("skills/clearance-register/digest.md"), live, "the overlay still wins — nothing changes on deploy");
  rmSync(root, { recursive: true, force: true });
});

test("a file deleted from the overlay falls through to the repo — one editable home, no drift", () => {
  const { root, baseDir, overlayDir } = trees();
  const repo = put(baseDir, "clearance-register/digest.md", "REPO COPY");
  const resolve = makeResolver({ overlayDir, baseDir });
  assert.equal(resolve("skills/clearance-register/digest.md"), repo);
  rmSync(root, { recursive: true, force: true });
});

test("customer-specific material stays overlay-only and is still found (it never enters the repo)", () => {
  const { root, baseDir, overlayDir } = trees();
  const fw = put(overlayDir, "clearance-search/risk-framework-acme.md", "ACME framework");
  const resolve = makeResolver({ overlayDir, baseDir });
  assert.equal(resolve("skills/clearance-search/risk-framework-acme.md"), fw);
  rmSync(root, { recursive: true, force: true });
});
...
test("absolutizeSkillRefs routes EACH reference independently through the resolver", () => {
  const { root, baseDir, overlayDir } = trees();
  put(baseDir, "clearance-register/digest.md", "REPO");
  put(baseDir, "clearance-search/synthesis-rules.md", "REPO");
  put(overlayDir, "clearance-search/risk-framework-acme.md", "OVERLAY");
  const resolve = makeResolver({ overlayDir, baseDir });
  const msg = "Read skills/clearance-register/digest.md and skills/clearance-search/risk-framework-acme.md and skills/clearance-search/synthesis-rules.md.";
  const out = absolutizeSkillRefs(msg, overlayDir, resolve);
  assert.ok(out.includes(join(dirname(baseDir), "skills/clearance-register/digest.md")), "generic → repo");
  assert.ok(out.includes(join(dirname(overlayDir), "skills/clearance-search/risk-framework-acme.md")), "customer → overlay");
  assert.ok(out.includes(join(dirname(baseDir), "skills/clearance-search/synthesis-rules.md")), "generic → repo");
  rmSync(root, { recursive: true, force: true });
});
...
test("a configured-but-unreadable overlay FAILS LOUD instead of silently using repo defaults", async () => {
  ...
  try {
    assert.throws(() => config.resolveSkillPath("skills/clearance-register/digest.md"), /skills_overlay_unreadable/);
  } finally {
