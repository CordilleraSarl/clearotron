...
const KO_CTX = Object.freeze({
  ...
  frameworkPath: "skills/clearance-search/risk-framework-triage.md",
  probeNote: null,
});
...
function unionFor(stage, artifacts) {
  ...
  //
  // register-unit's dispatch orders, in prose: "Read and follow skills/clearance-register/providers/<name>.md
 ...
 * `skills/clearance-register/providers/<name>.md` at runtime), so it cannot assert a capability of a
...
function providerConditionalSurfacesFor() {
  ...
  return new Set([`pointer:driver/skills/clearance-register/providers/${REGISTER_PROVIDER}.md`]);
}
...
test("directions (a) and (c) hold for EVERY stage, not only the recording category", () => {
  ...
  // `clearance-register/unit.md` now says exactly that, for `register_record_fetch` and
...
test("a tool named in a multi-reader doc is granted to every reader of it", () => {
  ...
  // documents involved are explicitly multi-mode: `clearance-register/SKILL.md` says in its own header that
...
test("ANTI-ROT: every marker still matches the live dispatch it was written for", () => {
  ...
  const unit = readFileSync(join(DRIVER, "skills/clearance-register/unit.md"), "utf8");
