...
const MD = "/r/clearance-search/x/common-law-findings.half-m.md";
const FORM = "/r/clearance-search/x/common-law-dispositions.half-m.json";
...
test("arm 1 — B: the connotation family grades at the stage's own output — there is no seat file to name", () => {
  ...
  assert.equal(gradedArtifact("connotation_call_partial:call_partial=3", "/r/clearance-search/x/common-law-findings.half-a.md"),
    "/r/clearance-search/x/common-law-findings.half-a.md", "…for every member, half or canonical");
});
...
test("arm 5 — gradedArtifact and repairTarget agree, because they are one derivation", () => {
  ...
  for (const [reason, out] of [
    ...
    ["coverage_no_status:no_status=3", "/r/clearance-search/x/register-findings.md"],
    ["coverage_axis_invalid:all axes", "/r/clearance-search/x/register-findings.md"],
    ["framediff_severity_invalid:major", "/r/clearance-search/x/frame-diff.md"],
  ]) {
...
test("arm 6 — the attempt row carries the FORM's sha, and two attempts that moved it record two", async () => {
  ...
  const r = await runStage("test-stage", {
    agent: "mailagent", message: "BASE TASK", sessionKey: "clearotron-graded-1",
...
test("arm 7 — the refusal the seat is handed names the form", async () => {
  ...
  const r = await runStage("test-stage", {
    agent: "mailagent", message: "BASE TASK", sessionKey: "clearotron-graded-2",
    timeoutSec: 30, maxRetries: 2, runDir: dir, expectFile: s.expectFile, validate: s.validate,
  });
...
test("arm 8 — the quarantine lane still recognises a finding token after the substitution", () => {
  ...
  const narrative = "/r/clearance-search/x/report.md";
...
test("arm 9 — the repair anchor did NOT move: `file` is still the stage's own output", {
  ...
  await runStage("test-stage", {
    agent: "mailagent", message: "BASE TASK", sessionKey: "clearotron-graded-3",
    timeoutSec: 30, maxRetries: 2, runDir: dir, expectFile: s.expectFile, validate: s.validate,
  });
...
test("arm 11 — a stage that owns no form records none, rather than echoing its own output", async () => {
  ...
  const r = await runStage("test-stage", {
    agent: "mailagent", message: "BASE TASK", sessionKey: "clearotron-graded-4",
...
const FINDINGS_M = "clearance-search/tmp8729-sample/2026-01-01-specimen/common-law-findings.half-m.md";
