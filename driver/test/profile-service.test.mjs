...
// zephyr/demo-brand-owner silently flipped to the house-default framework.
function svcWithFramework() {
  const dir = mkdtempSync(join(tmpdir(), "profile-svc-"));
  writeFileSync(join(dir, "generic.json"), JSON.stringify({ name: "House default", platforms: ["amazon.com"] }));
  writeFileSync(join(dir, "zephyr.json"), JSON.stringify({
    name: "Zephyr Beverages", platforms: ["amazon.com"],
    frameworkPath: "skills/clearance-search/risk-framework-zephyr.md",
    workedExamplesPath: "skills/clearance-search/worked-examples-zephyr.md",
    delivery: { email: "summary", privileged: true, style: "plain, direct", template: "standard" },
  }));
  writeFileSync(join(dir, "demo-brand-owner.json"), JSON.stringify({
    name: "Demo Brand Owner", platforms: ["amazon.com"],
...
test("save PRESERVES the framework selection when the client body omits it (the 2026-07-04 regression)", async () => {
  ...
  assert.equal(r.status, 200);
  assert.equal(writeCalls[0].profile.frameworkPath, "skills/clearance-search/risk-framework-zephyr.md",
    "an omitted frameworkPath is preserved from disk — never dropped");
  assert.equal(writeCalls[0].profile.workedExamplesPath, "skills/clearance-search/worked-examples-zephyr.md");
...
test("save IGNORES a client-supplied framework selection — on-disk wins, and none can be introduced", async () => {
  ...
  await service.route("POST", "/profiles/zephyr/save", STAFF,
    { profile: { name: "Zephyr Beverages", platforms: ["amazon.com"], frameworkPath: "skills/clearance-search/risk-framework.md" } });
  assert.equal(writeCalls[0].profile.frameworkPath, "skills/clearance-search/risk-framework-zephyr.md",
  ...
  await service.route("POST", "/profiles/generic/save", STAFF,
    { profile: { name: "House default", platforms: ["amazon.com"], frameworkPath: "skills/clearance-search/risk-framework-zephyr.md" } });
...
// ANTI-SILENT-FAILURE: these parse the decks actually shipped in skills/clearance-search/, so a future deck
// edit that breaks extraction fails CI here instead of silently blanking the page's "What the bands mean" box.
test("view lifts bandMeanings from the REAL shipped decks — band sections (house/zephyr); the matrix demo deck states none", async () => {
  const { service } = svcWithFramework();

  // demo (matrix-shaped) carries no Band-meanings table, so its box is absent; the table's rows are
  // driven through extractBandMeanings below with an invented table.
  const ms = (await service.route("GET", "/profiles/demo-brand-owner", STAFF)).json.framework;
  assert.equal(ms.bandMeanings ?? null, null, "the demo deck states no Band-meanings table, so it shows no box");
...
// `risk-framework-demo.md` and `risk-framework-zephyr.md` under exactly the names customers use. What
...
test("a framework served by the repository while a store is configured is reported, once", async () => {
  ...
  try {
    // the store holds zephyr's pair and NOT demo-brand-owner's — one customer's deck present, one gone
    ...
    for (const f of ["risk-framework-zephyr.md", "risk-framework-zephyr.manifest.json"])
      writeFileSync(join(store, "skills", "clearance-search", f),
        readFileSync(join(DRIVER_ROOT, "skills", "clearance-search", f), "utf8"));
    process.env.CLEAROTRON_INSTRUCTIONS_DIR = join(store, "skills");

    const { service } = svcWithFramework();
    const a1 = (await service.route("GET", "/profiles/demo-brand-owner", STAFF)).json.framework;
    // THE PAGE STILL WORKS, which is the whole problem — the substitution is invisible on the surface
    assert.ok(a1.manifest?.bands?.length, "the shipped deck renders perfectly well; nothing on the page is wrong");
    ...
    assert.ok(swapped.every((l) => /risk-framework-demo/.test(l)), "it names the framework that was substituted");

    const afterFirst = swapped.length;
    await service.route("GET", "/profiles/demo-brand-owner", STAFF);
    await service.route("GET", "/profiles/demo-brand-owner", STAFF);
...
test("bandMeanings is best-effort: a missing framework asset serves the view WITHOUT the key — never a 500", async () => {
  ...
  writeFileSync(join(dir, "ghost.json"), JSON.stringify({
    ...
    frameworkPath: "skills/clearance-search/risk-framework-ghost.md",   // valid shape, no such deck/manifest
  }));
...
test("extractBandMeanings: ANY miss ⇒ null (no partial box); garbled input never throws", () => {
  const msManifest = loadFrameworkManifest(DRIVER_ROOT, "skills/clearance-search/risk-framework-demo.md");
  const houseManifest = loadFrameworkManifest(DRIVER_ROOT, "skills/clearance-search/risk-framework.md");
  // No shipped matrix deck states a Band-meanings table, so the demo deck carries an invented one here.
  const msDeck = readFileSync(join(DRIVER_ROOT, "skills/clearance-search/risk-framework-demo.md"), "utf8") + "\n"
    + ["## Band meanings", "", "| Band | Meaning | Typical Response |", "|---|---|---|",
      ...msManifest.bands.map((b) => `| **${b.label}** | meaning of ${b.label} | response to ${b.label} |`)].join("\n") + "\n";
  const houseDeck = readFileSync(join(DRIVER_ROOT, "skills/clearance-search/risk-framework.md"), "utf8");
  ...
  assert.ok(cut > 0, "fixture: the matrix deck carries its Band meanings table");
...
function svcP({ gitCommit: gitCommitOver = null } = {}) {
  ...
  writeFileSync(join(dir, "generic.json"), JSON.stringify({ name: "House default", platforms: ["amazon.com"] }));
  writeFileSync(join(dir, "demo-brand-owner.json"), JSON.stringify({
    name: "Demo Brand Owner Corporation", matchDomains: ["demo-brand-owner.example"], platforms: ["store.steampowered.com", "itch.io"],
...
test("spec 62 GET /profiles/:customer/projects lists the customer's projects; unknown customer → 404", async () => {
  ...
  assert.deepEqual((await service.route("GET", "/profiles/demo-brand-owner/projects", STAFF)).json.projects, [], "empty at first");
  seedProject(dir, "demo-brand-owner", "japan-and-korea-app-launch", { projectName: "Japan and Korea app launch", platforms: ["amazon.com"] });
  const r = await service.route("GET", "/profiles/demo-brand-owner/projects", STAFF);
  ...
  assert.deepEqual(r.json.projects, [{ key: "japan-and-korea-app-launch", name: "Japan and Korea app launch", archived: false }]);
...
test("spec 62 GET /profiles/:customer/projects/:project serves overlay + inherited + effective + origins", async () => {
  const { service, dir } = svcP();
  seedProject(dir, "demo-brand-owner", "console", { projectName: "Japan and Korea app launch", platforms: ["amazon.com", "walmart.com", "ebay.com"], defaultClasses: [9, 28] }, "# ctx\nConsole peripherals.");
  const r = await service.route("GET", "/profiles/demo-brand-owner/projects/console", STAFF);
  assert.equal(r.status, 200);
  assert.equal(r.json.customerName, "Demo Brand Owner Corporation");
  assert.deepEqual(r.json.overlay, { projectName: "Japan and Korea app launch", platforms: ["amazon.com", "walmart.com", "ebay.com"], defaultClasses: [9, 28] }, "overlay is deltas only");
  ...
  assert.equal((await service.route("GET", "/profiles/demo-brand-owner/projects/nope", STAFF)).status, 404);
});
...
test("spec 62 save/validate: a valid overlay writes to projects/<c>/<p>.json, commits AS identity, audits", async () => {
  ...
  const ok = await service.route("POST", "/profiles/demo-brand-owner/projects/console/validate", STAFF,
    { profile: { projectName: "Japan and Korea app launch", platforms: ["amazon.com", "walmart.com"], defaultClasses: [9, 28] } });
  assert.equal(ok.json.ok, true);
  assert.equal(ok.json.isNew, true);
  assert.equal(writeCalls.length, 0, "validate never writes");
  const saved = await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF,
    { profile: { projectName: "Japan and Korea app launch", platforms: ["amazon.com", "walmart.com"], defaultClasses: [9, 28] },
      contextPack: "Console peripherals landscape.", author: "evil@attacker.test" });
  assert.equal(saved.status, 200);
  assert.equal(saved.json.written, true);
  assert.equal(writeCalls[0].customer, "demo-brand-owner");
  assert.equal(writeCalls[0].project, "console");
  assert.equal(commitCalls[0].author, "staff@example-firm.com", "git author is the verified identity, never the body");
  assert.equal(auditCalls[0].event, "project-create");
  assert.equal(auditCalls[0].key, "demo-brand-owner/console");
});
...
test("spec 62 save REJECTS a customer-only key (400) — the sparse validator, NOT the silent preserve path", async () => {
  ...
  const r = await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF,
    { profile: { projectName: "Console", name: "Japan and Korea app launch", platforms: ["amazon.com"] } });
  assert.equal(r.status, 400);
  assert.equal(r.json.error, "validation_failed");
...
test("spec 62 project guards: bad slug → 400; bad method → 405; unknown sub-action → 404; missing body → 400", async () => {
  const { service } = svcP();
  assert.equal((await service.route("POST", "/profiles/demo-brand-owner/projects/Bad_Slug/save", STAFF, { profile: { projectName: "X" } })).status, 400, "an unsafe project slug is rejected before any write");
  assert.equal((await service.route("PUT", "/profiles/demo-brand-owner/projects/console/save", STAFF, { profile: {} })).status, 405);
  assert.equal((await service.route("POST", "/profiles/demo-brand-owner/projects/console/bogus", STAFF, {})).status, 404);
  assert.equal((await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF, {})).status, 400, "no overlay object in the body");
});
...
test("archive: state is STICKY against omission — a body without `archived` cannot silently un-archive", async () => {
  ...
  seedProject(dir, "demo-brand-owner", "console", { archived: true, projectName: "Console", platforms: ["amazon.com"] });
...
test("archive: the commit message encodes the transition — archive / update / create", async () => {
  ...
  await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF,
    { profile: { projectName: "Console", platforms: ["amazon.com"] } });
  assert.match(commitCalls[0].message, /create project demo-brand-owner\/console/);

  seedProject(dir, "demo-brand-owner", "console", { projectName: "Console", platforms: ["amazon.com"] });
  await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF,
    { profile: { archived: true, projectName: "Console", platforms: ["amazon.com"] } });
  assert.match(commitCalls[1].message, /archive project demo-brand-owner\/console/, "live → archived reads as an archive");

  seedProject(dir, "demo-brand-owner", "console", { archived: true, projectName: "Console", platforms: ["amazon.com"] });
  await service.route("POST", "/profiles/demo-brand-owner/projects/console/save", STAFF,
    { profile: { archived: true, projectName: "Console 2", platforms: ["amazon.com"] } });
  assert.match(commitCalls[2].message, /update project demo-brand-owner\/console/, "an ordinary save of an already-archived project is still an update");
});
...
test("saving a SHOWN-THROUGH profile preserves its code-owned fields — the omitted-field class on the override-by-name path", async () => {
  ...
  assert.equal(writeCalls[0].profile.frameworkPath, "skills/clearance-search/risk-framework.md",
