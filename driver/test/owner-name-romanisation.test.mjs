...
test("the native-script owner survives the provider boundary instead of being collapsed away", () => {
...
test("an owner whose ONLY name is the native form has no separate native field", () => {
...
test("a Latin-only owner has no native form, and nothing changes for it", () => {
...
test("REC.ownerNative is provider-blind, like REC.owner", () => {
...
test("a romanised owner keeps the run's resolved rendering, with the raw form retained as provenance", () => {
...
test("THE GUARD THAT MUST NOT WEAKEN: a model-invented owner is still overwritten by the record", () => {
...
test("a romanised owner with NO resolved rendering falls back to the raw form — never blank", () => {
...
test("a whitespace-only rendering is not a rendering", () => {
  const findings = finding("   ");
  bindFindingsToRecords(findings, withRecord({ owner: PINYIN, ownerNative: NATIVE }));
  assert.equal(findings[0].owner.name, PINYIN);
});

test("with no record set at all it stays a NO-OP — an archived replay is not re-bound", () => {
  const findings = finding(ENGLISH);
...
test("reference-integrity joins the action list to the card once the heading is readable", () => {
...
test("the RENDERED report shows the resolved name for a romanised owner, not the pinyin", () => {
  const findings = finding(ENGLISH);
...
test("the RENDERED report still shows the RECORD's name for a Latin-only owner", () => {
...
test("render and the data layer cannot disagree — both ask the same function", () => {
...
test("a REPUBLISH of an archived run is byte-identical — no delivered report is rewritten", () => {
