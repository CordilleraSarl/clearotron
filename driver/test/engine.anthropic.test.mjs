...
/** the most recent turn, so a timing arm's failure can carry the specimen. */
let lastRun = null;
...
test("runStage: rate-limit -> fail 'rate_limited' (NOT nonzero_exit), carries resetsAt, ONE attempt (no retry/cascade)", async () => {
  ...
  try {
    const r = await runStage("matter-frame", { agent: "mailagent", message: "do it", model: "opus", thinking: "high", sessionKey: "clearotron-test-rl", timeoutSec: 60, maxRetries: 2 });
...
// A TEST FOR A SECOND ARGV BUILDER USED TO SIT HERE, and it is deleted with the builder. It
...
test("C2 GROUP KILL: the stall kill reaps a SIGTERM-ignoring grandchild (MCP-server stand-in) via SIGTERM→SIGKILL escalation", timed(async () => {
  ...
  // this arm reaped NOTHING: it asserts the fixtures are dead, which is true whenever the code
...
test("engine registry: CLEAROTRON_AI selects a provider adapter; a second adapter plugs in without touching runStage", () => {
  ...
  try {
    // SELECTION IS PROCESS-WIDE. This loop used to pass a stage name and read as a per-stage
...
test("NO-PROGRESS watchdog: a turn whose TOOL CALL NEVER RETURNS is still killed (#1624)", timed(async () => {
  ...
  assert.equal(r.toolCalls >= 1 || r.toolWaitMs > 0, true,
    ...
    // THE SPECIMEN, IN THE FAILURE MESSAGE. This arm has reddened `main` twice on exactly this
...
test("a STARVED SPAWN does not read as a no-progress stall — the kill lands after the child speaks", timed(async () => {
  ...
  // ── THE ARM STATES THE GRACE IT NEEDS, instead of trusting the box's ───────
  //
...
test("startup-class death: a CLI exit with NO stream events carries the noStreamEvents signal + a named stderr diagnosis", {
  ...
  //
  // the stand-in is BUILT, not borrowed from the filesystem. This read `/bin/false` and the
...
test("INTEGRATION: runStage on CLEAROTRON_AI=anthropic-agent writes file + returns ok via the real retry ladder", async () => {
  ...
  try {
    const r = await runStage("teststage", {
      agent: "mailagent", sessionKey: "clearotron-test-abc-matterframe",
...
// ── THE BIT THAT DISCRIMINATES THE CAUSE, PINNED ────────────────────────────────────────────
