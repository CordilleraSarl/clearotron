...
const subjectOn = (surface) => ({
  ...
  providerConditionalSurfaces: new Set(["pointer:driver/skills/clearance-register/providers/acmereg.md"]),
});
...
test("⭐ PLANT: the ACTIVE PROVIDER'S OWN DECK is a carve-out by construction — no phrase required", () => {
  ...
  const f = agreementFindings(subjectOn("pointer:driver/skills/clearance-register/providers/acmereg.md"));
  assert.deepEqual(f, [],
...
test("⭐ PLANT: the SAME sentence in provider-independent doctrine is NOT excused", () => {
  ...
  const f = agreementFindings(subjectOn("skill:driver/skills/clearance-register/unit.md"));
