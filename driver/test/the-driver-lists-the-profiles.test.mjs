...
test("the rank is ordinal against the run's own manifest, and an unlisted band is rank 0", () => {
...
test("a rank cut lists the ordinals at or above it", () => {
...
test("EVERY UNREADABLE STATE IS 'ALL' — wrong toward depth, never toward brevity", () => {
...
test("a BAND-LESS finding is INCLUDED and counted — absence of a key is not permission to skip", () => {
...
test("a cut that keeps EVERYTHING emits nothing — byte-identical where it changes nothing", () => {
...
test("a cut that keeps NOTHING says so explicitly — an empty list is not an absent one", () => {
...
test("the per-product rows carry the cut, and one-country does not", () => {
...
test("THE CALL SITE computes it on ctx, so the recheck sends the SAME list", () => {
