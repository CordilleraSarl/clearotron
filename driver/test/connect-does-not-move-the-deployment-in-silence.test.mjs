...
test("writing a different tree is a MOVE, and both paths are named", () => {
...
test("a FIRST write is not a move — an installer doing its job says so quietly", () => {
...
test("writing the SAME tree is not a move and says nothing at all", () => {
...
test("the running tree comes off the process's own command line", () => {
...
test("a RELATIVE entrypoint yields no tree, because its base is the process's cwd", () => {
...
test("a service running from another tree is a CONFLICT, named by unit and tree", () => {
...
test("the mixed box from the finding: three units on the old tree, one already moved", () => {
...
test("every service already on the destination tree is CLEAR", () => {
...
test("nothing readable is UNKNOWN, never a clear coast", () => {
...
test("a unit whose ExecStart names no module under the checkout is not guessed at", () => {
  const move = checkoutMove("/old", "/new");
  const p = movePosture({ move, running: [
...
test("a live process whose command line does not carry its unit's entrypoint is not evidence", () => {
...
test("no move means no question — the posture short-circuits", () => {
...
test("doctor names a program executing a DIFFERENT checkout than the install", () => {
...
test("doctor reports an unread process table as UNKNOWN, never as a clean box", () => {
...
test("doctor says so rather than passing when it has nothing to compare against", () => {
...
test("a box whose programs are all on the install's own tree is current", () => {
  const r = programsFromAnotherCheckout({
