...
test("a multi-mark summary names EVERY per-mark document by its real filename", () => {
...
test("it says there is NO combined report, because that is the fact that surprised a reader", () => {
...
test("the counts are NOT restated here — they are rendered once, in the documents", () => {
...
test("a single-mark run points at its one document and does not talk about others", () => {
...
test("NO DOCUMENTS ⇒ NO SECTION — an empty heading would assert an absence it cannot explain", () => {
...
test("the audit workbook line is omitted when there is no workbook, not printed empty", () => {
  const out = knockoutDocumentRoutes(MULTI).join("\n");
  assert.ok(!/audit workbook/.test(out));
  assert.ok(!/undefined|null/.test(out), "and no placeholder leaks into a delivered file");
});

test("a band-less document still routes — the route is not gated on the rating", () => {
