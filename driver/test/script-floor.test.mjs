...
test("item 21 — the stage that CAN satisfy the floor is told about it, at both prompt levels", async () => {
  ...
  const P = paths("/r");
  const jx = STAGES["clearance-variants"].message({ paths: P, job: { marks: ["NOVAPULSE"], jurisdictions: ["US", "CN", "JP"] }, profile: null });
  ...
  const latin = STAGES["clearance-variants"].message({ paths: P, job: { marks: ["NOVAPULSE"], jurisdictions: ["US", "GB"] }, profile: null });
