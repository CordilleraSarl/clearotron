...
test("EVERY grid refusal the server returns after the spec is read goes through the wrapper", () => {
  ...
  const preParse = src.slice(src.indexOf("if (grid_spec_path) {"), src.indexOf("if (!underStudioSegment(spec.output_path))"));
  ...
  const grid = src.slice(src.indexOf("if (!underStudioSegment(spec.output_path))"), src.indexOf("if (!task || task.trim().length === 0)"));
