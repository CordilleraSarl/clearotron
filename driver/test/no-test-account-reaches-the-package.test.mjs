...
//
// The last gate before a package is published.
//
...
test("a name in the PATH is found even when the bytes are unreadable", () => {
...
test("WHOLE-WORD: a name inside a longer word is not a hit", () => {
  assert.deepEqual(scanEntries([{ path: "x.md", text: "the vantablackbird sings" }], ["Vantablack"]), []);
});

test("CASE-INSENSITIVE: the lowercase form in a path is the same disclosure", () => {
  assert.equal(scanEntries([{ path: "a/vantablack/b.json", text: "" }], ["Vantablack"]).length, 1);
});

test("a clean package is CLEAN — the refusals are not universal", () => {
...
test("the manifest's own EXCLUSION line is not a mention of what it excludes", () => {
...
test("a list that cannot be used is a COULD-NOT-LOOK, never an empty list", () => {
...
test("the real shipped list loads and is usable, so the could-not-looks are not the only outcome", () => {
...
test("the exemption clears the LINE and not the FILE", () => {
...
test("a BUNDLED DEPENDENCY's own tests are not ours, and refusing on them refuses every release", () => {
...
test("the package is refused when it carries no house risk framework to rate under", () => {
...
test("a FIXTURE framework is not the house default — the match is exact, not a substring", () => {
...
test("a path that merely CONTAINS the word test is not a fixture", () => {
