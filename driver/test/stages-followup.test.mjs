...
const CUSTOM = {
  frameworkPath: "skills/clearance-search/risk-framework-acme.md",
  workedExamplesPath: "skills/clearance-search/worked-examples-acme.md",
};
...
test("A-1 — the profile-aware stages resolve the CUSTOMER's framework, not the static default", () => {
  for (const name of ["synthesis", "report-overview", "report-card"]) {
    ...
    assert.ok(!resolved.includes("skills/clearance-search/risk-framework.md"),
  ...
  assert.ok(resolveSkillReads("synthesis", { profile: null, job: { classes: ["5"] } })
    .includes("skills/clearance-search/field-doctrine-pharma.md"), "the pharma force-read reaches a corrective pass too");
});
