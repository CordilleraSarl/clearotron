...
test("warmEligible truth table + patch message carries the reason-aware hint", () => {
  ...
  assert.match(warmPatchMessage("invalid_file:x:use_check_missing:F1", ["/r/clearance-search/x/narrative.md"]),
    /Use-check source/);
});
...
test("warm patch write-mode: invalid_file patches in place, missing_file still writes the complete file", () => {
  ...
  const absent = warmPatchMessage("missing_file:x/out.md", ["/r/clearance-search/x/out.md"]);
...
test("warm patch on a multi-file stage aims the repair at the member the token names", () => {
  const files = ["/r/clearance-search/x/common-law-findings.half-a.md", "/r/clearance-search/x/common-law-findings.half-b.md"];
  const m = warmPatchMessage("invalid_file:x/common-law-findings.half-b.md:platforms_missing:etsy", files);
  assert.match(m, /TARGETED EDITS to \/r\/clearance-search\/x\/common-law-findings\.half-b\.md/);
  assert.doesNotMatch(m, /TARGETED EDITS to \/r\/clearance-search\/x\/common-law-findings\.half-a\.md/,
