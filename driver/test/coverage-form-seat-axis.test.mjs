...
const DIGEST_MD = readFileSync(join(HERE, "..", "skills", "clearance-register", "digest.md"), "utf8");
...
test("HALF 2 — the skill's worked examples are THREE, and every axis it teaches is in the vocabulary", () => {
  // digest.md named FOUR until the ⭐ floor was retired: the per-jurisdiction reconciliation, the ⭐-floor sweep, the
...
const AXIS_TOKEN = `invalid_file:clearance-search/tmp9004-mark/run/register-findings.md:`
...
test("the repair grades at the stage's own output — no coverage file is a target any more", () => {
  ...
  const dg = "/r/clearance-search/tmp9004-mark/run/register-findings.md";
