...
test("a HOSTED box with no PORTAL_MCP_URL is the incident, and it is a fault", () => {
...
test("a SUPERVISED box with no PORTAL_MCP_URL is NOT a fault — the value never enters a shell", () => {
...
test("A CREDENTIAL IS NOT A LANE — a token with no address is half-wired, and says so", () => {
...
test("A CONFIGURED VALUE IS NOT A WALKED HOP — unprobed is reported as unprobed", () => {
...
test("an origin carrying a path is refused — the client appends /mcp itself", () => {
...
test("a configured lane whose door does not answer names the reason, whichever way it failed", () => {
...
test("a token that can start and cannot stop is a FAULT — a lane is both directions", () => {
...
test("an ABSENT verbs claim is full ops and must not red", () => {
...
test("both verbs present is the shape the default install mints", () => {
...
test("a missing address outranks a narrow token — the bigger fault is reported first", () => {
