...
test("the fingerprint is over the ITEMS, not their order or their repetition in one call", () => {
...
test("a call naming no items has no fingerprint and can never be a repeat", () => {
...
test("the idField is REQUIRED - a hash over the wrong property matches nothing, silently", () => {
...
test("the GENERATION KEY splits a match, which is what makes a positional id safe", () => {
...
test("listGeneration is CONTENT-derived, so a rewrite of the identical list does not split it", () => {
...
test("the positional receiver passes a generation and the others deliberately do not", () => {
