...
test("a key inside an ALREADY-TYPED object is reported, with its path", () => {
...
test("array items are walked, and the path names WHICH row", () => {
...
test("a clean call reports nothing, so the census is not satisfied by always firing", () => {
  assert.deepEqual(undeclaredKeys(SCHEMA, { narrative: { spine: "…", verdict: "…" }, rows: [{ ordinal: 1 }] }), []);
});

test("an object the schema declares WITHOUT properties reports nothing — stated, not hidden", () => {
...
test("a non-object where the schema wants one is not walked into and does not throw", () => {
