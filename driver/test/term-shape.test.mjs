...
// term:"WAVO*"} ×4 and dispatched frame-diff display LABELS as mark terms — both came back
...
test("hasAnchoredWildcard: anchored stars only — infix stars are legal marks, `?` is never plan syntax", () => {
  assert.equal(hasAnchoredWildcard("WAVO*"), true);
  assert.equal(hasAnchoredWildcard("*WAVO"), true);
  assert.equal(hasAnchoredWildcard("*WAVO*"), true);
  ...
  assert.equal(hasAnchoredWildcard("WAVO"), false);
...
test("termPredicateIssue: an anchored-star term under a LITERAL predicate is the 2026-07-28 false-clean class", () => {
  for (const p of ["exact", "default", "phonetic"]) {
    assert.match(termPredicateIssue("WAVO*", p) ?? "", /false clean/i, `${p} + WAVO* must be an issue`);
  ...
  assert.match(termPredicateIssue("WAVO", "wildcard") ?? "", /neither `\*` nor `\?`/);
  // and the agreeing pairs are clean
  assert.equal(termPredicateIssue("WAVO", "exact"), null);
  assert.equal(termPredicateIssue("WAVO*", "wildcard"), null);
  assert.equal(termPredicateIssue("*WAVO*", "wildcard"), null);
...
test("termShapeIssue: label/prose-shaped strings fire; marks — including 3-4 word marks — never do on word count alone", () => {
  ...
  assert.match(termShapeIssue("Reverse-order WAVO composites (TROPICAL WAVO, ISLAND WAVO)") ?? "", /label\/prose-shaped/);
  ...
  assert.match(termShapeIssue("WAVU, WAPO one-keystroke neighbours of WAVO") ?? "", /words/);
  // punctuation signals on 3-4 word strings
  assert.match(termShapeIssue("WAVO family (composites)") ?? "", /parenthetical/);
  assert.match(termShapeIssue("WAVO — the family") ?? "", /sentence punctuation/);
  assert.match(termShapeIssue("WAVO BAR; SLUSH") ?? "", /sentence punctuation/);
  assert.match(termShapeIssue("CORAL / FREEZE") ?? "", /space-flanked slash/);
  // genuine marks never fire
  assert.equal(termShapeIssue("WAVO"), null);
...
test("entryTermIssues: the entry-level walk — owner predicate exempt, term_literal is the escape hatch", () => {
  ...
  for (const term of ["WAVO*", "*WAVO", "SLUSH*", "*SLUSH"]) {
  ...
  const stack = entryTermIssues({ predicate: "exact", terms: ["WAVO", "SLUSH*", "WAVO POP"] });
  ...
  assert.deepEqual(entryTermIssues({ predicate: "wildcard", term: "WAVO*" }), []);
  assert.deepEqual(entryTermIssues({ predicate: "default", term: "WAVO" }), []);
});
...
test("the discriminators are markup and the enumeration — NEVER the bracket", () => {
  ...
  // bullet: `*WAVO` is a legitimate variant, so no leading-bullet arm may exist here.
