...
test("an instance with NOTHING configured still lists every provider, each marked missing", () => {
...
test("a capability row carries a remedy instead of a credential, and says which it is", () => {
...
test("the same three rows are present when everything IS configured, and say so", () => {
...
test("a missing credential names the VARIABLE and never reads a value out", () => {
...
test("EVERY required variable, not just the first: euipo needs two and both are named", () => {
...
test("no register selected is a row naming the SELECTOR, in this release's spelling", () => {
...
test("a register id this build does not ship is a TYPO, not an unmade choice", () => {
...
test("the register row carries the vendor's own label, never a title-cased id", () => {
...
test("the engine reports the vendor, the id, and a subscription bill", () => {
...
test("an unset engine reports the DEFAULT, not an empty one", () => {
...
test("THE THROW IS THE ROW: api-key billing with no key is recorded, not swallowed", () => {
...
test("api-key billing WITH the key is billed to the key", () => {
...
test("an engine this build does not ship is marked unknown rather than rendered as a name", () => {
...
test("a binary that cannot be found is false, not a throw and not a path", () => {
...
test("the engine id resolves by the RUN DOOR's expression, whitespace included", () => {
...
test("engineMode derives the three states, and growth in neither direction changes that", () => {
...
test("NOTHING short of a completed probe produces `engine-ready`", () => {
