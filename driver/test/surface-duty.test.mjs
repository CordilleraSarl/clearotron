...
test("a STATED ground is an answer — the arm no archived run can provide", () => {
...
test("ONE silent decline trips it — there is no threshold to tune", () => {
...
test("`step-structural` is NOT silence — an upstream absence is not a judging stage's fault", () => {
...
test("both silence tokens count, because both leave the reader unable to tell", () => {
...
test("the same record twice counts once, and casing is not a second record", () => {
...
test("no rows is not a pass — an empty run reports zero surfaced, and says so", () => {
...
test("the disclosure names what cannot be told apart, not a bare count", () => {
