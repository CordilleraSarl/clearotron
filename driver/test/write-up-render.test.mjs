...
test("a short entry is recognisable as INTENDED, never as a truncated full card", () => {
...
test("NO NEW AUTHORED PROSE — the sentence is the typed net, with the archived fallback", () => {
...
test("multi-country groups by TERRITORY, and a mark in three appears under all three", () => {
...
test("a finding with NO jurisdiction is never put in an `unknown` group", () => {
...
test("minimum group size is TWO — a single-member group renders as a short entry", () => {
...
test("WORLDWIDE groups by the right — one row, territories listed, not one row per country", () => {
...
test("a by-right label unions EVERY member's territories, never just the first placed", () => {
...
test("ZERO IS NOT ABSENCE — an empty section says so rather than going silent", () => {
...
test("the member line is the grammar the reasoned-negatives section already ships", () => {
