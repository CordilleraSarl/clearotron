...
//
// This test is the enforcement the boundary rule in skills/clearance-register/providers/README.md always
// stated but nothing checked — and which was, in fact, already being violated by SKILL.md and
// clearance-search/phase2-execution.md before the neutral-namespace change.
//
...
//   - skills/clearance-register/providers/  — the provider vocabulary docs, which exist to be provider-specific
...
const isAllowedFile = (rel) =>
  ...
  rel.startsWith("skills/clearance-register/providers/");
...
test("no vendor tool tokens outside the provider glue and provider vocabulary docs", () => {
  ...
  assert.deepEqual(offenders, [],
    ...
    `skills/clearance-register/providers/. Use the neutral register_* names — they resolve to whichever ` +
...
// `clearance-register/digest.md` and `clearance-search/delivery-contract.md` went on naming the Corsearch
...
test("no provider-agnostic skill file states a record base host", () => {
  ...
  const PROVIDER_DOCS = join("skills", "clearance-register", "providers");

  const hosts = new Set();
  for (const path of walked(join(SKILLS, "clearance-register", "providers"))) {
...
test("tracker 2018 the walk refuses an empty corpus, and an empty leaf is not one", () => {
  ...
  const tree = mkdtempSync(join(tmpdir(), "b2018-provider-neutral-tree-"));
  ...
  try {
    ...
    const baseline = walked(root).map((f) => relative(root, f)).sort();
    assert.deepEqual(baseline, ["one.mjs"], "the laid-out tree is not the tree the walk reads — this arm would prove nothing");
    mkdirSync(leaf, { recursive: true });
    assert.deepEqual(walked(root).map((f) => relative(root, f)).sort(), baseline,
