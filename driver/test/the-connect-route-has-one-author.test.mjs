...
test("the command is the one CONNECT.md documents, with this install's own path", () => {
  ...
  // On Windows the product quotes the separator, which PowerShell would otherwise swallow, and the
  // server's path carries the platform's separator. Both spellings are pinned exactly.
  assert.match(cmd, process.platform === "win32"
    ? /"--" \S*node\S* .*mcp-server\\serve\.mjs$/
    : /-- \S*node\S* .*mcp-server\/serve\.mjs$/, "it must end at the server's entry, run by this install's own Node");
