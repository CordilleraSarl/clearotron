...
test("config.skillsRoot follows CLEAROTRON_INSTRUCTIONS_DIR — the driver reads frameworks where the AGENT reads them", async () => {
  ...
  mkdirp(joinPath(store, "skills", "clearance-search"), { recursive: true });
  const fwPath = "skills/clearance-search/risk-framework-tenant.md";
  ...
  const houseManifest = JSON.parse(readFileSync(joinPath(HERE_DRIVER, "skills/clearance-search/risk-framework.manifest.json"), "utf8"));
