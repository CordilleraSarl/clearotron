...
function repo({ branch = "main" } = {}) {
  ...
  writeFileSync(join(root, "skills", "clearance-search", "digest.md"), "committed doctrine\n");
...
test("an uncommitted edit under the served path fails", () => {
  const { store } = repo();
  writeFileSync(join(store, "clearance-search", "digest.md"), "draft nobody committed\n");
  const r = classifySkillsStore(store);
...
test("an UNTRACKED file under the served path fails too — a stray doctrine file matches no commit", () => {
  const { store } = repo();
  writeFileSync(join(store, "clearance-search", "stray.md"), "nobody's document\n");
  const r = classifySkillsStore(store);
  assert.equal(r.outcome, "fail");
  assert.deepEqual(r.findings, ["dirty"]);
});
...
test("a store on a feature branch fails, and names the branch", () => {
  ...
  writeFileSync(join(store, "clearance-search", "digest.md"), "branch doctrine\n");
  git(root, "add", "-A");
...
test("a detached HEAD off the main line fails — detached is not collapsed into either answer", () => {
  ...
  writeFileSync(join(store, "clearance-search", "digest.md"), "side doctrine\n");
  git(root, "add", "-A");
...
test("both faults at once are both reported", () => {
  ...
  git(root, "add", "-A");
  git(root, "commit", "-qm", "branch work");
  writeFileSync(join(store, "clearance-search", "digest.md"), "and an uncommitted draft on top\n");
  const r = classifySkillsStore(store);
  assert.equal(r.outcome, "fail");
...
test("the door WARNS by default and REFUSES only under the strict flag", () => {
  const { store } = repo();
  writeFileSync(join(store, "clearance-search", "digest.md"), "draft\n");
