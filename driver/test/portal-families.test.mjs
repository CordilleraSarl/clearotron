...
test("ONE CUSTOMER'S FAMILY NEVER ADOPTS ANOTHER'S — same name, different owner, refused", () => {
  ...
  const root = poolWith({ ours: "demo-brand-owner", theirs: "borealis" });
  groupRuns(root, { name: "Core range", runIds: ["ours"], account: "demo-brand-owner" });
...
test("a family is scoped on READ as well as on write", () => {
  ...
  const root = poolWith({ ours: "demo-brand-owner", theirs: "borealis" });
  groupRuns(root, { name: "Ours", runIds: ["ours"], account: "demo-brand-owner" });
  groupRuns(root, { name: "Theirs", runIds: ["theirs"], account: "borealis" });

  const mine = familiesView(root, "demo-brand-owner");
...
test("ungrouping removes the run and drops a family left with nothing in it", () => {
  ...
  view = familiesView(root, "demo-brand-owner");
...
test("A MALFORMED SIDECAR DEGRADES TO NO FAMILIES — it never takes the list down", () => {
  ...
  assert.deepEqual(familiesView(root, "demo-brand-owner"), { of: {}, names: {} });
...
test("a nameless family is refused rather than minted with a blank heading", () => {
  ...
  for (const name of ["", "   ", "!!!", null, undefined]) {
    assert.ok(groupRuns(root, { name, runIds: ["a"], account: "demo-brand-owner" }).error, `${JSON.stringify(name)} refused`);
  }
});
...
const poolOf = (metas) => {
  ...
  for (const [runId, meta] of Object.entries(metas)) {
    mkdirSync(join(root, runId), { recursive: true });
    writeFileSync(join(root, runId, "meta.json"), JSON.stringify({ runId, customerKey: "demo-brand-owner", ...meta }));
  }
  return root;
};
