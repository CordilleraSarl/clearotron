...
**A conflict whose proprietor cannot be identified on the record searched has no band at all.** Not a low one: the framework rates a legal read, and a legal read needs a party — who holds the right, what they have done with it, whether they would act. With the owner unknown none of that can be reasoned, so a band placed anyway is a number standing in for the work of finding out, and it reads to the client as though somebody had looked. A delivered run rated such a conflict Low as "distinguished on the mark itself" while its own off-register pass had met the title four times without ever looking it up, and the pending family behind it was found later by the reviewing lawyer. Carry it in the actions as an open item that names the identification step — who the proprietor is, and what is pleaded — and leave it out of the rated findings and out of the reasoned negatives, which are the two places a reader takes as settled.
