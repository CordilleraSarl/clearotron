...
3. the variant manifest's **`### Scope ledger`** section (the variants-stage coverage statement, spanning the variant / field / source layers — see [clearotron-variants/SKILL.md](../clearotron-variants/SKILL.md#scope-ledger)).
...
|---|---|
| **Common-law** | Surfaced by `clearotron-common-law` only. Marketplace presence, no register evidence. |
| **Register** | Surfaced by `clearotron-register` only. Filed protection, no visible marketplace use yet (or no marketplace evidence captured). |
...
`clearotron-register/digest.md` (A1) dictates. A register finding with no grounding registration is an **orphan** —
...
The record-URL contract lives in `clearotron-register/status-rules.md`. **The half you need is here** because
