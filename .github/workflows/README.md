...
| `windows.yml` | The same install, doctor and demo on Windows from PowerShell, plus Ctrl-C on `clearotron start` and a whole clearance on each engine against stand-in engine programs. | Daily, by hand, and from the release on every beta and stable; a red run holds a stable. |
