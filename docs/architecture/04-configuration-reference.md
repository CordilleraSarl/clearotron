...
| `CLAUDE_CODE_USE_VERTEX` / `CLAUDE_CODE_USE_FOUNDRY` / `CLAUDE_CODE_USE_BEDROCK` | **Read by the Claude program**, which sends every turn to that cloud; `1`, `true`, `yes` or `on` switches one on. Clearotron reads them only to name the cloud under `CLEAROTRON_AI_BILLING=cloud` and to refuse a switch left on under `subscription` or `api-key`. Doctor and setup's proof turn carry these and the four rows below from the settings file, where a value in the shell wins over the file's and, in setup, an answer given wins over both (`CLOUD_SETTINGS` in `driver/engine/auth.mjs`); a run takes the whole file. |
