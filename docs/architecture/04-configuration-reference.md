...
| `CLEAROTRON_START_ENV_FILE` | unset | **Never set this in a file.** An internal handoff: `clearotron start` gives its queue runner the path of the env file it read, so a search refused at order time names the file the runner's values came from. It is handed at the spawn and never written to the units' file, which is how a unit's runner still names its own. |
