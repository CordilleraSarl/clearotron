...
| `PORTAL_READ_MODEL` | `claude-sonnet-5` | The model the portal's own compose-read turn uses. Distinct from the pipeline's tiers: this is a portal surface, not a stage. It names a version because this turn calls the API directly rather than through the CLI, and the API takes model ids, not tier words — so unlike a stage it does **not** follow the vendor to a newer model, and moving it is an edit here. The same is true of the native-language lanes' default. |
