...
| `CLAUDE_CODE_OAUTH_TOKEN` | **Read by the Claude program itself**, not by Clearotron — the stage subprocess inherits it untouched in every billing mode, and that inheritance is the whole mechanism by which a server with no browser authenticates the subscription lane. `claude setup-token`, run on any machine you can sign in on, prints it; setup captures it by paste. Clearotron names it only in the refusal a missing sign-in produces, so the reader can paste the fix. |
