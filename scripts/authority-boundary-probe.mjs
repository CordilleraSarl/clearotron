...
mkdirSync(join(skills, "clearance-register"), { recursive: true });
ensureDriverDir(runDir);
writeFileSync(join(skills, "clearance-register", "SKILL.md"), "# doctrine\nThe method for this stage.\n");
const doctrine = join(skills, "clearance-register", "NOTES.md");   // the incident's shape: a NEW file in the doctrine tree
const existing = join(skills, "clearance-register", "SKILL.md");   // already on disk — the Edit target
