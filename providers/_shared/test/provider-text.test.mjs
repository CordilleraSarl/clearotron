...
test("THE DEFECT, IN ONE ASSERTION: the old head-clip severs the discriminator", () => {
...
test("the tail-preserving clip keeps the word the classifier needs", () => {
...
test("at the shipped budget the refusal is not clipped at all", () => {
...
test("a message that fits comes back byte-identical, whatever its length", () => {
  for (const s of ["", "short", "x".repeat(139), "x".repeat(140)]) {
    assert.equal(clipProviderText(s, 140), s);
  }
});

test("the budget holds at every length, and the tail is always the input's tail", () => {
...
test("degenerate budgets do not produce something that is mostly ellipsis", () => {
