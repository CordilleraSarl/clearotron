...
test("the count probe names a refused query with its term count, its size and the declared width", async () => {
  const { out, sent } = await withFetch(500, WRAPPED, () =>
    doCount("k", "https://x", { names: names(500), regions: ["US"], match_mode: "exact" }, null));
  const text = JSON.stringify(out);
  const value = sent.find((s) => s.url.endsWith("/count"))?.body?.searchFields?.[0]?.value ?? "";
  assert.equal(value.split(" OR ").length, 500, "the stub did not see the 500-term stack");
  ...
  assert.match(text, new RegExp(`Sent 500 OR terms \\(${value.length} characters\\); this provider's declared width is ${CAPABILITIES.maxOrWidth} terms\\.`));
...
test("a run's enumerate records the named refusal, not a bare 500", async () => {
  const { out } = await withFetch(500, WRAPPED, () =>
    doEnumerate("k", "https://x", { names: names(500), regions: ["US"], match_mode: "exact" }, null));
  ...
  assert.match(reason, /Sent 500 OR terms \(\d+ characters\)/, `the numbers did not survive the enumerate wrapper: ${reason}`);
});
...
test("the plan executor defers a refused query at once, as a capability gap, and asks nothing more about it", async () => {
  // The service's bound, stubbed: a stack wider than it is refused, a narrower one counts.
  ...
  try {
    const plan = { regions: ["US"], entries: [
      { qid: "q-wide", axis: "primary-sweep", predicate: "exact", terms: names(500), nice_classes: [9] },
    ...
    assert.match(wide.reason, /Sent 500 OR terms \(\d+ characters\); this provider's declared width is \d+ terms\.$/,
    ...
    assert.equal(counts.get(500), 2, `the refused query was sent ${counts.get(500)} times`);
